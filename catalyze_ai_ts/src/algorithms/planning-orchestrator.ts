...
export class PlanningOrchestrator {
  ...
  console.log(`[Orchestrator] roundTasks count=${roundTasks.length} sample=${JSON.stringify(roundTasks.slice(0,3))}`);
    ...
    // 3. 日次タスクの割り当て
    const dailyTasks = this._allocateDailyTasks(
      plan,
      sessions,
      roundTasks,
      quota.adjustedTimePerUnitMs,
      usedDailyQuota
    );
    // eslint-disable-next-line no-console
  ...
  const rangeEnd = plan.unitRange?.end ?? plan.totalUnits;
    ...
    const remainingRoundTasks: RoundTask[] = [];
  ...
  // 逆算分配: 締切までの学習日を列挙し、残りユニットを日ごとに分配する
    ...
    const perDayAssign: number[] = new Array(daysCount).fill(Math.max(perDayCeil, minQuotaCeil));
    ...
    for (; dayIndex < daysCount && currentTaskIndex < remainingRoundTasks.length; dayIndex++) {
      ...
      let dayStartUnit: number | null = null;
      let dayEndUnit: number | null = null;
      ...
      while (remainingToAssign > 0 && currentTaskIndex < remainingRoundTasks.length) {
        ...
        // clamp
        if (startUnit < rangeStart) startUnit = rangeStart;
        if (endUnit > rangeEnd) endUnit = rangeEnd;
